...
export const EntityNode3D: React.FC<{
    entity: EntityData;
    node: { globalPos: THREE.Vector3; landSize: number; level: number; color: string };
}> = ({ entity, node }) => {
    ...
    return (
        <group position={[node.globalPos.x, node.globalPos.y, node.globalPos.z]}>
            ...
            <Html
                transform
                position={[0, 0.75, buildingZ + 1.55]}
                style={{
                    color: 'white',
                    fontWeight: 'bold',
                    fontSize: '18px',
...
export const InternalFlash: React.FC<{
    ...
    onComplete: () => void;
    durationMs?: number;
}> = ({ pos, scale, label, color, onComplete, durationMs = 800 }) => {
...
export const ExternalFlow: React.FC<{
    ...
    onComplete: () => void;
    durationMs?: number;
}> = ({ startPos, endPos, label, color, scale, onComplete, durationMs = 1200 }) => {
    ...
    const curve = useMemo(() => {
        ...
        const height = Math.max(p1.y, p4.y) + 3;

        const p2 = new THREE.Vector3(p1.x, height, p1.z);
        const p3 = new THREE.Vector3(p4.x, height, p4.z);
    ...
    return (
        <>
            ...
            <Html
                position={[midPoint.x, midPoint.y + 0.7 * scale, midPoint.z]}
                center
                style={{
                    color,
                    fontWeight: 'bold',
